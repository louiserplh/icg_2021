...
for this task, we ask TAs about what exactly we had to return, 
and what was represented by each argument.
we found out that if we did the dot product with the normal of the plane and
if the normal was going toward us then the dot product was negative 
=> we changed the sign.
We found t with the formula from the course and then we checked our calculations 
...
we used overleaf to code TheoryExercise_1.
...
Tom found the final solution ( because we had problems developping the second degree equation)
...
we implemented separate functions, one for the infinite cylinder and one to cut betweet the two caps.
